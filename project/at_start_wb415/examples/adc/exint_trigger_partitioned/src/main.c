...
/** @addtogroup 415_ADC_current_vref_value_check ADC_current_vref_value_check
  * @{
  */


__IO uint16_t adc1_ordinary_valuetab[3][2] = {0};
__IO uint16_t adc1_preempt_valuetab[3][2] = {0};
...
static void gpio_config(void)
{
  ...
  gpio_initstructure.gpio_pins = GPIO_PINS_0 | GPIO_PINS_1 | GPIO_PINS_3 | GPIO_PINS_4;
...
static void adc_config(void)
{
  ...
  adc_ordinary_conversion_trigger_set(ADC1, ADC12_ORDINARY_TRIG_EXINT11_TMR1TRGOUT, TRUE);
  adc_dma_mode_enable(ADC1, TRUE);
  adc_ordinary_part_count_set(ADC1, 1);
  adc_ordinary_part_mode_enable(ADC1, TRUE);

  adc_preempt_channel_length_set(ADC1, 2);
  adc_preempt_channel_set(ADC1, ADC_CHANNEL_3, 1, ADC_SAMPLETIME_239_5);
  adc_preempt_channel_set(ADC1, ADC_CHANNEL_4, 2, ADC_SAMPLETIME_239_5);
  adc_preempt_conversion_trigger_set(ADC1, ADC12_PREEMPT_TRIG_EXINT15_TMR1CH4, TRUE);
