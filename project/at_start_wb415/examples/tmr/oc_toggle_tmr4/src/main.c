...
/** @addtogroup 415_TMR_oc_toggle_tmr4 TMR_oc_toggle_tmr4
  * @{
  */
...
__IO uint16_t ch3_val = 8192;
__IO uint16_t ch4_val = 4096;
...
void gpio_configuration(void)
{
  ...
  /* gpioa configuration:tmr4 channel1, 2, 3 and 4 as alternate function push-pull */
...
int main(void)
{
  ...
  /* tmr4 configuration: output compare toggle mode:
     tmr4clk = system_core_clock,
     the objective is to get tmr4 counter clock at 24 mhz:
     - div = (tmr4clk / tmr4 counter clock) - 1
     cc1 update rate = tmr4 counter clock / ch1_val / 2 = 366.2 hz
     cc2 update rate = tmr4 counter clock / ch2_val / 2 = 732.4 hz
     cc3 update rate = tmr4 counter clock / ch3_val / 2 = 1464.8 hz
     cc4 update rate = tmr4 counter clock / ch4_val / 2 = 2929.6 hz */

  /* compute the div value */
  div_value = (uint16_t)(system_core_clock / 24000000) - 1;

  /* tmre base configuration */
  tmr_base_init(TMR4, 65535, div_value);
  ...
  tmr_channel_value_set(TMR4, TMR_SELECT_CHANNEL_1, ch1_val);

  /* output compare toggle mode configuration: channel2 */
  tmr_output_channel_config(TMR4, TMR_SELECT_CHANNEL_2, &tmr_oc_init_structure);
  tmr_channel_value_set(TMR4, TMR_SELECT_CHANNEL_2, ch2_val);
  ...
  /* tmr it enable */
  tmr_interrupt_enable(TMR4, TMR_C1_INT | TMR_C2_INT | TMR_C3_INT | TMR_C4_INT, TRUE);
