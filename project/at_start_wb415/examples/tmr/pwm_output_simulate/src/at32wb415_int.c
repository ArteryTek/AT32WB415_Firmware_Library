...
extern __IO uint16_t ch1_val;
extern __IO uint16_t ch2_val;
extern __IO uint16_t ch3_val;
extern __IO uint16_t ch4_val;

/** @addtogroup AT32WB415_periph_examples
  * @{
  */

/** @addtogroup 415_TMR_pwm_output_simulate
  * @{
  */
...
void TMR2_GLOBAL_IRQHandler(void)
{
  if(tmr_flag_get(TMR2, TMR_C1_FLAG) != RESET)
  {
    ...
    gpio_bits_write(GPIOB, GPIO_PINS_6, (confirm_state)(1 - gpio_output_data_bit_read(GPIOB, GPIO_PINS_6)));
  ...
  else if(tmr_flag_get(TMR2, TMR_C2_FLAG) != RESET)
  {
    ...
    /* Pin PC.06 toggling with frequency = 73.24 Hz */
    gpio_bits_write(GPIOB, GPIO_PINS_7, (confirm_state)(1 - gpio_output_data_bit_read(GPIOB, GPIO_PINS_7)));
  ...
  else if(tmr_flag_get(TMR2, TMR_C3_FLAG) != RESET)
  {
    ...
    /* Pin PC.06 toggling with frequency = 73.24 Hz */
    gpio_bits_write(GPIOB, GPIO_PINS_8, (confirm_state)(1 - gpio_output_data_bit_read(GPIOB, GPIO_PINS_8)));
    capture = tmr_channel_value_get(TMR2, TMR_SELECT_CHANNEL_3);
    tmr_channel_value_set(TMR2, TMR_SELECT_CHANNEL_3, capture + ch3_val);
  }
  else
  {
    ...
    /* Pin PC.06 toggling with frequency = 73.24 Hz */
    gpio_bits_write(GPIOB, GPIO_PINS_9, (confirm_state)(1 - gpio_output_data_bit_read(GPIOB, GPIO_PINS_9)));
    capture = tmr_channel_value_get(TMR2, TMR_SELECT_CHANNEL_4);
    tmr_channel_value_set(TMR2, TMR_SELECT_CHANNEL_4, capture + ch4_val);
  }
}
