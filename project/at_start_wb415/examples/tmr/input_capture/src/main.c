/**
  **************************************************************************
  * @file     main.c
  * @version  v2.0.1
  * @date     2022-05-20
  * @brief    main program
  **************************************************************************
  *                       Copyright notice & Disclaimer
  *
  * The software Board Support Package (BSP) that is made available to
  * download from Artery official website is the copyrighted work of Artery.
  * Artery authorizes customers to use, copy, and distribute the BSP
  * software and its related documentation for the purpose of design and
  * development in conjunction with Artery microcontrollers. Use of the
  * software is governed by this copyright notice and the following disclaimer.
  *
  * THIS SOFTWARE IS PROVIDED ON "AS IS" BASIS WITHOUT WARRANTIES,
  * GUARANTEES OR REPRESENTATIONS OF ANY KIND. ARTERY EXPRESSLY DISCLAIMS,
  * TO THE FULLEST EXTENT PERMITTED BY LAW, ALL EXPRESS, IMPLIED OR
  * STATUTORY OR OTHER WARRANTIES, GUARANTEES OR REPRESENTATIONS,
  * INCLUDING BUT NOT LIMITED TO, THE IMPLIED WARRANTIES OF MERCHANTABILITY,
  * FITNESS FOR A PARTICULAR PURPOSE, OR NON-INFRINGEMENT.
  *
  **************************************************************************
  */

#include "at32wb415_board.h"
#include "at32wb415_clock.h"

/** @addtogroup AT32WB415_periph_examples
  * @{
  */

/** @addtogroup 415_TMR_input_capture TMR_input_capture
  * @{
  */

gpio_init_type  gpio_init_struct = {0};
crm_clocks_freq_type crm_clocks_freq_struct = {0};
tmr_input_config_type  tmr_input_config_struct;
void uart_init(uint32_t baudrate);
__IO uint32_t tmr4freq = 0;
__IO uint32_t sys_counter = 0;
__IO uint16_t capturenumber = 0;
__IO uint16_t ch3readvalue1 = 0, ch3readvalue2 = 0;
__IO uint32_t capture = 0;

/**
  * @brief  this function handles tmr4 trigger exception.
  * @param  none
  * @retval none
  */
void TMR4_GLOBAL_IRQHandler(void)
{
  if(tmr_flag_get(TMR4, TMR_C2_FLAG) != RESET)
  {
    tmr_flag_clear(TMR4, TMR_C2_FLAG);
    if(capturenumber == 0)
    {
      /* get the Input Capture value */
      ch3readvalue1 = tmr_channel_value_get(TMR4, TMR_SELECT_CHANNEL_2);
      capturenumber = 1;
    }
    else if(capturenumber == 1)
    {
      /* get the Input Capture value */
      ch3readvalue2 = tmr_channel_value_get(TMR4, TMR_SELECT_CHANNEL_2);

      /* capture computation */
      if(ch3readvalue2 > ch3readvalue1)
      {
        capture = (ch3readvalue2 - ch3readvalue1);
      }
      else
      {
        capture = ((0xFFFF - ch3readvalue1) + ch3readvalue2);
      }

      /* frequency computation */
      tmr4freq = (uint32_t) crm_clocks_freq_struct.sclk_freq / capture;
      capturenumber = 0;
    }
  }
}

/**
  * @brief  main function.
  * @param  none
  * @retval none
  */
int main(void)
{
  system_clock_config();

  at32_board_init();

  uart_print_init(115200);

  /* get system clock */
  crm_clocks_freq_get(&crm_clocks_freq_struct);

  /* turn led2/led3/led4 on */
  at32_led_on(LED2);
  at32_led_on(LED3);
  at32_led_on(LED4);

  /* enable tmr4/gpioa clock */
  crm_periph_clock_enable(CRM_TMR4_PERIPH_CLOCK, TRUE);
  crm_periph_clock_enable(CRM_GPIOB_PERIPH_CLOCK, TRUE);

  /* timer3 input pin Configuration */
  gpio_init_struct.gpio_pins = GPIO_PINS_7;
  gpio_init_struct.gpio_mode = GPIO_MODE_INPUT;
  gpio_init_struct.gpio_out_type = GPIO_OUTPUT_PUSH_PULL;
  gpio_init_struct.gpio_pull = GPIO_PULL_NONE;
  gpio_init_struct.gpio_drive_strength = GPIO_DRIVE_STRENGTH_STRONGER;
  gpio_init(GPIOB, &gpio_init_struct);

  /* tmr4 configuration: input capture mode
     the external signal is connected to tmr4 ch2 pin (pb.07)
     the rising edge is used as active edge,
     the tmr4 c2dt is used to compute the frequency value */

  /* tmr4 counter mode configuration */
  tmr_base_init(TMR4, 0xFFFF, 0);
  tmr_cnt_dir_set(TMR4, TMR_COUNT_UP);

  /* configure tmr4 channel2 to get clock signal */
  tmr_input_config_struct.input_channel_select = TMR_SELECT_CHANNEL_2;
  tmr_input_config_struct.input_mapped_select = TMR_CC_CHANNEL_MAPPED_DIRECT;
  tmr_input_config_struct.input_polarity_select = TMR_INPUT_RISING_EDGE;
  tmr_input_channel_init(TMR4, &tmr_input_config_struct, TMR_CHANNEL_INPUT_DIV_1);

  tmr_interrupt_enable(TMR4, TMR_C2_INT, TRUE);

  /* tmr2 trigger interrupt nvic init */
  nvic_priority_group_config(NVIC_PRIORITY_GROUP_4);
  nvic_irq_enable(TMR4_GLOBAL_IRQn, 0, 0);

  /* enable tmr4 */
  tmr_counter_enable(TMR4, TRUE);

  while(1)
  {
    delay_ms(100);
    printf("The external signal frequece is : %d\r\n",tmr4freq);
    tmr4freq = 0;
  }
}

/**
  * @}
  */

/**
  * @}
  */
