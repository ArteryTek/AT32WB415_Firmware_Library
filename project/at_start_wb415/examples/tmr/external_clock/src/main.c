...
/** @addtogroup 415_TMR_external_clock TMR_external_clock
  * @{
  */
...
int main(void)
{
  ...
  /* enable tmr2/tmr4/gpioa/gpiob clock */
  ...
  /* use tmr2 generate clock to drive tmr4
  tmr2 pwm frequency = 144000000(systemclock)/144(period+1)/1000(div+1) = 1 khz
  tmr4 pwm frequency = 1000(tmr2 pwm frequency)/10(period+1)/1(div+1) = 100 hz */

  /* tmr2 and tmr4 counter mode configuration */
  tmr_base_init(TMR2, 143, 999);
  tmr_cnt_dir_set(TMR2, TMR_COUNT_UP);

  tmr_base_init(TMR4, 9, 0);
  tmr_cnt_dir_set(TMR4, TMR_COUNT_UP);

  /* tmr2 and tmr4 output mode configuration */
  ...
  tmr_channel_value_set(TMR2, TMR_SELECT_CHANNEL_1, 72);
  /* tmr4 channel 3 */
  tmr_output_channel_config(TMR4, TMR_SELECT_CHANNEL_3, &tmr_output_struct);
  tmr_channel_value_set(TMR4, TMR_SELECT_CHANNEL_3, 5);
