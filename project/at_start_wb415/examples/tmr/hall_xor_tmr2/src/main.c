...
/** @addtogroup 415_TMR_hall_xor_tmr4 TMR_hall_xor_tmr4
  * @{
  */
...
int main(void)
{
  ...
  /* enable tmr4/gpioa/gpiob clock */
  crm_periph_clock_enable(CRM_TMR4_PERIPH_CLOCK, TRUE);
  crm_periph_clock_enable(CRM_GPIOA_PERIPH_CLOCK, TRUE);
  crm_periph_clock_enable(CRM_GPIOB_PERIPH_CLOCK, TRUE);

  /* tmr4 channel1 ,channel2 and channel3 configuration */
  ...
  gpio_init_struct.gpio_pins = GPIO_PINS_3 | GPIO_PINS_4 | GPIO_PINS_5 | GPIO_PINS_8;
  ...
  /* tmr4 xor mode configuration
  tmr4 channel1 ,channel2 and channel3 as hall signal input channel, trigger
  generated each signal edge. */

  /* tmr4 counter mode configuration */
  tmr_32_bit_function_enable(TMR4, TRUE);

  tmr_base_init(TMR4, 0xFFFFFFFF, 0);
  ...
  /* xor funtion enable */
  tmr_channel1_input_select(TMR4, TMR_CHANEL1_2_3_CONNECTED_C1IRAW_XOR);

  /* select the tmr4 input trigger: C1INC */
  tmr_trigger_input_select(TMR4, TMR_SUB_INPUT_SEL_C1INC);

  /* select the slave mode: reset mode */
  tmr_sub_mode_select(TMR4, TMR_SUB_RESET_MODE);

  tmr_interrupt_enable(TMR4, TMR_TRIGGER_INT, TRUE);

  /* tmr4 trigger interrupt nvic init */
...
/**
  * @brief  this function handles TMR4 trigger exception.
  * @param  none
  * @retval none
  */
