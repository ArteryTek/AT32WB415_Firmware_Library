...
/** @addtogroup 415_TMR_oc_high TMR_oc_high
  * @{
  */
...
int main(void)
{
  ...
  div_value = (uint16_t)(crm_clocks_freq_struct.sclk_freq / 4000) - 1;

  /* tmr2 time base configuration */
  tmr_base_init(TMR2, 0xFFFF, div_value);
  ...
  tmr_channel_value_set(TMR2, TMR_SELECT_CHANNEL_2, ch2_val);

  tmr_output_channel_config(TMR2, TMR_SELECT_CHANNEL_3, &tmr_oc_init_structure);
  tmr_channel_value_set(TMR2, TMR_SELECT_CHANNEL_3, ch3_val);

  tmr_output_channel_config(TMR2, TMR_SELECT_CHANNEL_4, &tmr_oc_init_structure);
  tmr_channel_value_set(TMR2, TMR_SELECT_CHANNEL_4, ch4_val);
